    ...
    <div class="pr-0 md:pr-6 mt-5">
      ...
      <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-6">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
          <!-- Calendar -->
          <div class="flex items-center gap-2 bg-white rounded-lg shadow-sm border border-gray-200 px-3 py-2">
            <svg class="w-4 h-4 text-[#1a3a35]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <!-- Status Filter -->
          <select v-model="filterStatus" class="bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a3a35] shadow-sm">
          ...
          <!-- Type Filter -->
          <select v-model="filterType" class="bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a3a35] shadow-sm">
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gray-50 border-b border-gray-200">
                ...
                <th class="px-4 py-4 text-right text-xs font-bold text-gray-600 uppercase tracking-wider">
                ...
                <th class="px-4 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">
              ...
              <tr v-for="payment in filteredPayments" :key="payment.id" class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-4 whitespace-nowrap">
                  <div class="flex items-center gap-2">
                    <button class="w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center text-white text-xs hover:bg-blue-600 transition-colors">
                      +
                    </button>
                ...
                <!-- Provider (Resource) -->
                <td class="px-4 py-4 whitespace-nowrap">
                  <span class="text-sm text-gray-700">{{ payment.provider }}</span>
                </td>
                <!-- Service -->
                <td class="px-4 py-4 whitespace-nowrap">
                ...
                <!-- Appointment Date -->
                <td class="px-4 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-700">{{ payment.appointmentDate }}</div>
                  <div class="text-xs text-gray-400">{{ payment.appointmentTime }}</div>
                </td>
                <!-- Amount -->
                ...
                <!-- Subtotal -->
                <td class="px-4 py-4 whitespace-nowrap text-right">
                  <span class="text-sm font-semibold text-gray-900">{{ payment.subtotal }}</span>
                </td>
                <!-- Actions -->
                <td class="px-4 py-4 whitespace-nowrap text-center">
                  <div class="flex items-center justify-center gap-2">
                    <button class="flex items-center gap-1 bg-white border border-gray-300 text-gray-700 px-3 py-1.5 rounded-lg text-sm hover:bg-gray-50 transition-colors">
      ...
      <div class="flex justify-between items-center mt-6">
        <p class="text-sm text-gray-500">Showing {{ filteredPayments.length }} of {{ payments.length }} payments</p>
