<template>
  ...
  >
    <!-- left side -->
    <div
      class="relative rounded-lg col-span-1 bg-ttlocker-img h-[94vh] bg-cover bg-left lg:flex flex-col justify-between gap-6 hidden"
    >
    ...
    <div class="grid justify-items-center py-5">
      ...
      <div class="heding text-center mb-6 mt-20 sm:mt-0">
        ...
        <span class="text-[#6C6C6C] text-sm">TTLocker Access Portal</span>
      </div>
...
<script setup>
  ...
  import { TTLockerLoginOtp } from "@/services/apiService.js";
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (response.isSuccess) {
        ...
        router.push(`/ttlocker/otp?email=${encodeURIComponent(email.value)}`);
      } else {
        errorMessage.value =
