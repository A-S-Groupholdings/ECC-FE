  ...
  <div class="min-h-screen bg-[#faf9f7] flex items-center justify-center px-4">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-900">TT Locker Verification</h1>
...
<script setup>
  ...
  async function verifyOtp() {
    ...
    try {
      const response = await TTLockerVerifyOtp(email.value, otp);
      ...
      if (response.isSuccess) {
        ...
        if (data && data.user) {
          ...
          authStore.setUser({
            ...
            role: data.user.role,
            isTTLockerUser: data.user.isTTLockerUser === true,
          });

          // Store tokens in Pinia (accessToken in sessionStorage, refreshToken in cookie)
          authStore.setTokens(data.accessToken, data.refreshToken);

          if (data.user.isTTLockerUser === true) {
            router.push("/ttlocker/home");
          } else {
            errorMessage.value = "This account does not have TT Locker access.";
            otpDigits.value = ["", "", "", "", "", ""];
            otpInputs.value[0].focus();
          }
        } else {
          errorMessage.value = "Invalid response. Please try again.";
        }
      } else {
        errorMessage.value =
  ...
  async function resendOtp() {
    ...
    try {
      const response = await TTLockerLoginOtp(email.value);
